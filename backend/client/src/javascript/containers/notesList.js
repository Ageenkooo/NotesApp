                    ...
                    return (
                        <div key={note.id}>
                            <Div key={note.id} className='flex flex-center'>
                                <Item
                                    className={note.id === this.props.note.id
                                        ? 'chosen notepad color'
                                        : 'notepad color'}
                                    onClick={() => {this.props.actions.selectNote(note) }}
                                    key={note.id}
                                    name={note.name}>
                                    <ModalWindow
                                        trigger={<Cancel/>}
...
function mapStateToProps(state) {
    return {book: state.actBook, notes: state.notes, note: state.actNote};
}
